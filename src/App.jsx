...
const SLIDES = [
  ...
  { id:"creating",   label:"Creating Results",Component:S06, color:"#34d399" },
...
export default function App() {
  ...
  const go = i => { if(i===cur) return; setDir(i>cur?1:-1); setCur(i); };
  ...
  return (
    <div style={{
      width:"100vw", height:"100vh", overflow:"hidden", background:"#060912",
      ...
      <div style={{
        ...
        backgroundSize:"56px 56px", transition:"background-image 0.8s"
      }}/>
      {/* Glow orb */}
      <motion.div
        key={color}
      ...
      <div style={{
        ...
        borderBottom:`1px solid ${color}1e`, transition:"border-color 0.6s"
      }}>
        <div style={{ display:"flex", alignItems:"center", gap:7 }}>
          ...
          />
          <span style={{ color, fontSize:10, letterSpacing:"0.22em", textTransform:"uppercase", transition:"color 0.5s", fontFamily:"Fira Code, monospace" }}>
            Operation Result Pattern · .NET
          </span>
        </div>
        <div style={{ display:"flex", gap:4, alignItems:"center" }}>
          {SLIDES.map((s,i)=>(
            <button key={s.id} onClick={()=>go(i)} title={s.label} style={{
      ...
      {/* Arrows */}
      {cur>0 && (
        <motion.button onClick={prev} whileHover={{scale:1.12,x:-2}}
          style={{ position:"fixed", left:12, top:"50%", transform:"translateY(-50%)", zIndex:100,
            width:36, height:36, borderRadius:8, border:`1px solid ${color}28`,
            background:`${color}0a`, color, fontSize:18, display:"flex", alignItems:"center", justifyContent:"center" }}>‹</motion.button>
      )}
      {cur<SLIDES.length-1 && (
        <motion.button onClick={next} whileHover={{scale:1.12,x:2}}
